# OpenCodeMonitor iOS Remote Blueprint (Tailscale + TCP)

This document is the canonical runbook for iOS remote usage with a desktop-hosted OpenCodeMonitor backend over Tailscale.
...
1. Desktop OpenCodeMonitor hosts the daemon and executes OpenCode workflows.
2. iOS OpenCodeMonitor connects to the desktop daemon using `remoteBackendHost` + token.
...
- Desktop OpenCodeMonitor is installed and able to run local workspaces.
