...
export type OpenCodeArgsRecognizedSegment = {
  flag: string;
...
export type OpenCodeArgsOption = {
...
function normalizeOpenCodeArgs(value: string | null | undefined): string | null {
  return normalizeOpenCodeArgsInput(value);
}
...
export function parseOpenCodeArgsProfile(args: string | null | undefined): ParsedOpenCodeArgsProfile {
  const originalArgs = normalizeOpenCodeArgsInput(args) ?? "";
  if (!originalArgs) {
    return {
      originalArgs: "",
      recognizedSegments: [],
...
function fallbackLabelFromParsed(parsed: ParsedOpenCodeArgsProfile): string {
  return fallbackLabel(parsed.effectiveArgs ?? parsed.originalArgs);
}

function buildOptionLabelFromParsed(parsed: ParsedOpenCodeArgsProfile): string {
...
export function buildOpenCodeArgsOptionLabel(args: string): string {
...
export function buildOpenCodeArgsOptions(input: {
  ...
  const options: OpenCodeArgsOption[] = [
    { value: "", opencodeArgs: null, label: "Default" },
  ];
