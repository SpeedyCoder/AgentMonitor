...
describe("useResizablePanels", () => {
  ...
  it("reads stored sizes and clamps to bounds", () => {
    window.localStorage.setItem("opencodemonitor.sidebarWidth", "999");
    window.localStorage.setItem("opencodemonitor.rightPanelWidth", "100");
    window.localStorage.setItem("opencodemonitor.planPanelHeight", "not-a-number");
  ...
  it("persists sidebar width changes and clamps max", () => {
    ...
    expect(hook.result.sidebarWidth).toBe(420);
    expect(window.localStorage.getItem("opencodemonitor.sidebarWidth")).toBe(
      "420",
    );
