...
pub(crate) fn read_personality() -> Result<Option<String>, String> {
    let Some(root) = resolve_default_opencode_home() else {
        return Ok(None);
    };
...
pub(crate) fn config_toml_path() -> Option<PathBuf> {
    resolve_default_opencode_home().map(|home| home.join("config.toml"))
}

pub(crate) fn read_config_model(opencode_home: Option<PathBuf>) -> Result<Option<String>, String> {
    let root = opencode_home.or_else(resolve_default_opencode_home);
    let Some(root) = root else {
        return Err("Unable to resolve OPENCODE_CONFIG_DIR".to_string());
    };
    let (_, document) = config_toml_core::load_global_config_document(&root)?;
    Ok(config_toml_core::read_top_level_string(&document, "model"))
}

fn resolve_default_opencode_home() -> Option<PathBuf> {
    crate::opencode::home::resolve_default_opencode_home()
}
