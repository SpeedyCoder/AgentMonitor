...
pub(crate) async fn get_app_settings_core(app_settings: &Mutex<AppSettings>) -> AppSettings {
    ...
    if let Ok(Some(collaboration_modes_enabled)) = config::read_collaboration_modes_enabled()
    {
        settings.collaboration_modes_enabled = collaboration_modes_enabled;
    }
    if let Ok(Some(steer_enabled)) = config::read_steer_enabled() {
        settings.steer_enabled = steer_enabled;
    }
    if let Ok(Some(unified_exec_enabled)) = config::read_unified_exec_enabled() {
        settings.unified_exec_enabled = unified_exec_enabled;
    }
    if let Ok(Some(apps_enabled)) = config::read_apps_enabled() {
        settings.experimental_apps_enabled = apps_enabled;
    }
    if let Ok(personality) = config::read_personality() {
...
pub(crate) async fn update_app_settings_core(
    ...
    let _ = config::write_collaboration_modes_enabled(settings.collaboration_modes_enabled);
    let _ = config::write_steer_enabled(settings.steer_enabled);
    let _ = config::write_unified_exec_enabled(settings.unified_exec_enabled);
    let _ = config::write_apps_enabled(settings.experimental_apps_enabled);
    let _ = config::write_personality(settings.personality.as_str());
