...
pub(crate) async fn opencode_update_core(
    ...
    let (default_bin, default_args) = {
        ...
        (settings.opencode_bin.clone(), settings.opencode_args.clone())
    };
    let resolved = opencode_bin
        .clone()
        .filter(|value| !value.trim().is_empty())
        .or(default_bin);
    let resolved_args = opencode_args
        .clone()
        .filter(|value| !value.trim().is_empty())
        .or(default_args);
    let _ = resolved_args;
    ...
    let (method, package, upgrade_ok, output, upgraded) = if detect_brew_cask("opencode").await? {
    ...
    } else if detect_brew_formula("opencode").await? {
