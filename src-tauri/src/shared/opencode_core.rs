...
pub(crate) enum OpenCodeLoginCancelState {
...
async fn resolve_opencode_home_for_workspace_core(
    workspaces: &Mutex<HashMap<String, WorkspaceEntry>>,
    workspace_id: &str,
) -> Result<PathBuf, String> {
    let (entry, parent_entry) = resolve_workspace_and_parent(workspaces, workspace_id).await?;
    resolve_workspace_opencode_home(&entry, parent_entry.as_ref())
...
pub(crate) async fn account_read_core(
    ...
    let opencode_home = resolve_workspace_opencode_home(&entry, parent_entry.as_ref())
        .or_else(resolve_default_opencode_home);
    let fallback = read_auth_account(opencode_home);

    Ok(build_account_response(response, fallback))
}

pub(crate) async fn opencode_login_core(
    sessions: &Mutex<HashMap<String, Arc<WorkspaceSession>>>,
    ...
    let (cancel_tx, cancel_rx) = oneshot::channel::<()>();
    {
        ...
        if let Some(existing) = cancels.remove(&workspace_id) {
            match existing {
                OpenCodeLoginCancelState::PendingStart(tx) => {
                    let _ = tx.send(());
                }
                OpenCodeLoginCancelState::LoginId(_) => {}
            }
        }
        cancels.insert(
            workspace_id.clone(),
            OpenCodeLoginCancelState::PendingStart(cancel_tx),
        );
    }
...

    {
        ...
        cancels.insert(
            workspace_id,
            OpenCodeLoginCancelState::LoginId(login_id.clone()),
        );
    }
...
pub(crate) async fn opencode_login_cancel_core(
    sessions: &Mutex<HashMap<String, Arc<WorkspaceSession>>>,
    opencode_login_cancels: &Mutex<HashMap<String, OpenCodeLoginCancelState>>,
    workspace_id: String,
) -> Result<Value, String> {
    ...
    match cancel_state {
        ...
        OpenCodeLoginCancelState::LoginId(login_id) => {
            let session = get_session_clone(sessions, &workspace_id).await?;
...
pub(crate) async fn remember_approval_rule_core(
    ...
    let rules_path = rules::default_rules_path(&opencode_home);
