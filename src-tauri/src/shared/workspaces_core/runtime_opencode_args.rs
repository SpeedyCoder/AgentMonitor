...
pub(crate) struct WorkspaceRuntimeOpenCodeArgsResult {
    pub(crate) applied_opencode_args: Option<String>,
    pub(crate) respawned: bool,
}

pub(crate) async fn set_workspace_runtime_opencode_args_core<F, Fut>(
    workspace_id: String,
    opencode_args_override: Option<String>,
    workspaces: &Mutex<HashMap<String, WorkspaceEntry>>,
    sessions: &Mutex<HashMap<String, Arc<WorkspaceSession>>>,
    app_settings: &Mutex<AppSettings>,
    spawn_session: F,
) -> Result<WorkspaceRuntimeOpenCodeArgsResult, String>
where
    ...
    Fut: Future<Output = Result<Arc<WorkspaceSession>, String>>,
{
    ...
    let target_args = opencode_args_override
        .as_deref()
        .map(str::trim)
    ...
    if !workspace_connected {
        return Ok(WorkspaceRuntimeOpenCodeArgsResult {
            applied_opencode_args: target_args,
            respawned: false,
        });
    }
    ...
    if current_session.opencode_args == target_args {
        return Ok(WorkspaceRuntimeOpenCodeArgsResult {
            applied_opencode_args: target_args,
            respawned: false,
        });
    ...
    let new_session =
        spawn_session(entry.clone(), default_bin, target_args.clone(), opencode_home).await?;
    let workspace_ids = {
    ...
    Ok(WorkspaceRuntimeOpenCodeArgsResult {
        applied_opencode_args: target_args,
        respawned: true,
    })
}
...
mod tests {
    ...
    fn make_session(_entry: WorkspaceEntry, opencode_args: Option<String>) -> WorkspaceSession {
    ...
    #[test]
    fn set_workspace_runtime_opencode_args_is_noop_when_workspace_not_connected() {
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            let result = set_workspace_runtime_opencode_args_core(
                entry.id.clone(),
            ...
            assert_eq!(
                result,
                WorkspaceRuntimeOpenCodeArgsResult {
                    applied_opencode_args: Some("--profile dev".to_string()),
                    respawned: false
                }
            );
    ...
    fn set_workspace_runtime_opencode_args_is_noop_when_args_match() {
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            ...
            assert_eq!(
                result,
                WorkspaceRuntimeOpenCodeArgsResult {
                    applied_opencode_args: Some("--same".to_string()),
                    respawned: false
                }
            );
            assert_eq!(spawn_calls.load(Ordering::SeqCst), 0);
    ...
    #[test]
    fn set_workspace_runtime_opencode_args_respawns_when_args_change() {
        tokio::runtime::Runtime::new().unwrap().block_on(async {
            let entry = make_workspace_entry("ws-1");
            let workspaces = Mutex::new(HashMap::from([(entry.id.clone(), entry.clone())]));
            ...
            assert_eq!(
                result,
                WorkspaceRuntimeOpenCodeArgsResult {
                    applied_opencode_args: Some("--new".to_string()),
                    respawned: true
                }
            );
