...
pub(super) async fn try_handle(
    ...
    match method {
        "get_opencode_config_path" => {
            let path = match settings_core::get_opencode_config_path_core() {
                Ok(value) => value,
        ...
        "set_opencode_feature_flag" => {
            ...
            Some(
                state
                    .set_opencode_feature_flag(feature_key, enabled)
                    .await
                    .map(|_| json!({ "ok": true })),
            )
        }
        ...
        "opencode_login" => {
            ...
            Some(state.opencode_login(workspace_id).await)
        }
        "opencode_login_cancel" => {
            ...
            Some(state.opencode_login_cancel(workspace_id).await)
        }
        "skills_list" => {
            let workspace_id = match parse_string(params, "workspaceId") {
                Ok(value) => value,
        ...
        "opencode_doctor" => {
            ...
            let opencode_args = parse_optional_string(params, "opencodeArgs");
            Some(state.opencode_doctor(opencode_bin, opencode_args).await)
        }
